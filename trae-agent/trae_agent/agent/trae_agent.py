...
class TraeAgent(BaseAgent):
    ...
    def get_system_prompt(self) -> str:
        ...
        return self._system_prompt_override or TRAE_AGENT_SYSTEM_PROMPT

    def set_system_prompt(self, prompt: str | None) -> None:
        if prompt is None:
            self._system_prompt_override = None
            return
        if prompt == "TRAE_AGENT_SYSTEM_PROMPT":
            self._system_prompt_override = TRAE_AGENT_SYSTEM_PROMPT
            return
        if prompt == "DOCUMENT_AGENT_SYSTEM_PROMPT":
            self._system_prompt_override = DOCUMENT_AGENT_SYSTEM_PROMPT
            return
    ...
    def reflect_on_result(self, tool_results: list[ToolResult]) -> str | None:
        if not tool_results:
            return None
        errs = [tr.error for tr in tool_results if not tr.success and tr.error]
        if errs:
            return "\n".join([f"工具执行失败：{e}" for e in errs])
        texts = [str(tr.result) for tr in tool_results if tr.success and tr.name and "quality_review" in tr.name and tr.result]
        if not texts:
            return None
        header = "质量审查结果："
        body = "\n\n".join(texts)
    ...
    def llm_indicates_task_completed(self, llm_response: LLMResponse) -> bool:
        ...
        if llm_response.tool_calls is None:
            # Allow direct Q&A completion when no patch is required
            # If there is a non-empty assistant answer and no tool calls, treat as done
            if self.must_patch != "true" and llm_response.content and llm_response.content.strip():
                return True
            return False
