...
class TrajectoryRecorder:
    ...
    def set_quality_review(self, enabled: bool, rules: str | None) -> None:
        self.trajectory_data.update(
            {
                "quality_review_enabled": bool(enabled),
                "quality_review_rules": rules or "1、内容必须切合主题。2、内容禁止过于单薄。3、内容必须具备合理的逻辑。",
            }
        )
        self.save_trajectory()
    ...
    def record_agent_step(
        ...
        existing_step_idx = next(
            (
                i
                for i, s in enumerate(self.trajectory_data["agent_steps"])
        ...
        if existing_step_idx is not None:
            # Preserve existing llm_messages if new is None
            if step_data["llm_messages"] is None:
                step_data["llm_messages"] = self.trajectory_data["agent_steps"][
                    existing_step_idx
                ].get("llm_messages")
            self.trajectory_data["agent_steps"][existing_step_idx] = step_data
        else:
    ...
    def save_trajectory(self) -> None:
        ...
        try:
            ...
            tmp_path = self.trajectory_path.with_suffix(self.trajectory_path.suffix + ".tmp")
        ...
        try:
            key = str(self.trajectory_path.resolve())
            listeners = _global_listeners.get(key) or []
            for cb in list(listeners):
                try:
                    cb(self.trajectory_data)
                except Exception:
                    pass
        except Exception:
            pass
...
_global_listeners: dict[str, list[Callable[[dict[str, Any]], None]]] = {}


def _add_global_listener_impl(path: str, callback: Callable[[dict[str, Any]], None]) -> None:
    try:
        key = str(Path(path).resolve())
        arr = _global_listeners.get(key) or []
        arr.append(callback)
        _global_listeners[key] = arr
    except Exception:
        pass


setattr(TrajectoryRecorder, "add_global_listener", staticmethod(_add_global_listener_impl))
